...
Extracts data from Census Bureau economic indicators
"""
...
def extract_census_mrts(api_key, start_date=START_DATE, categories=['441', '444', '4441']):
    ...
    if 'category_code' in df.columns:
        # Convert Series to string, then map
        cat_code_series = df['category_code'].iloc[:, 0] if df['category_code'].ndim > 1 else df['category_code']
        df['category_name'] = cat_code_series.astype(str).replace(category_names)
    else:
