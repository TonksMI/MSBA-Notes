    ...
    def create_categorical_figure(self, focus):
        ...
        fig = make_subplots(
            ...
            subplot_titles=(
                ...
                f'{focus.title()}: Contribution Analysis (Pareto)'
            ),
        ...
        fig.add_trace(
            go.Bar(x=sorted_depts, y=sorted_perf,
                   ...
                   text=[f'{p}%' for p in sorted_perf],
                   textposition='outside',
                   name=focus_desc, showlegend=False),
            row=1, col=1
        )

        # Add target line and zone
        target = 85
        ...
        # Add 80% line for Pareto principle (on secondary y-axis)
        fig.add_shape(type="line", x0=-0.5, x1=len(sorted_all_depts)-0.5, y0=80, y1=80,
                     line=dict(dash="dot", color="red"), yref="y4", row=2, col=1)
        fig.add_annotation(x=len(sorted_all_depts)-1, y=80, text="80% Rule",
                          showarrow=False, yref="y4", row=2, col=1)
        ...
        # Update axes
        fig.update_xaxes(showgrid=False, title_text="", tickangle=45)
        ...
        fig.update_yaxes(title_text="Cumulative %", secondary_y=False, row=2, col=1)
